...
# Scientific/ML library imports
from sklearn.decomposition import PCA

# NuScenes related imports
...
def calc_metrics(prediction, gt):
...
def visualize_bev(bev_embed, stream=False):
    # Stream
    if stream:
        bev_features = np.transpose(bev_embed, (1, 2, 0))      # 100, 50, 256
        bev_embed = np.flip(bev_features, axis=0).copy()  
    ...
    if stream:
        pca_image = principal_component.reshape(100, 50)
    else:
...
def main(args):
    ...
    for sample_token in sample_tokens_all:
        ...
        if scene_record['name'] in splits['val']:
    ...
    with open(args.bev_results, 'rb') as handle:
    ...
    for key, value in predict_data.items():
        ...
        if HiVT == True:
            ...
            data[int(key)]['predict_fut_bev'] = normalize_hivt(predict_data_bev[key], x, y, heading)
        ...
        # DenseTNT
            ...
            data[int(key)]['predict_fut_bev'] = np.flip(predict_data_bev[key].reshape(1, 6, 30, 2), -1)
    ...
    for first_index, tokens in enumerate(tqdm(scene_frames_all)):
        ...
        for second_index, token in enumerate(tokens):
            if (first_index, second_index) not in track:
                ...
                metric_bev_scene.append(calc_metrics_min(value['predict_fut_bev'][0], n_ego_fut))

        average_baseline = average_of_tuples(metric_baseline_scene)
        average_unc = average_of_tuples(metric_unc_scene)
        average_bev = average_of_tuples(metric_bev_scene)

        improves_unc = [(-(average_baseline[0] - average_unc[0])/(average_baseline[0]+0.00001)) * 100, 
        ...
        for i, improve in enumerate(improves_unc):
            if round(improve, 0) == 0:
                improves_unc[i] = '0%'
            else:
                if improve > 0:
                    improves_unc[i] = f'+{improve:.0f}%'
                elif improve < 0:
                    improves_unc[i] = f'{improve:.0f}%'


        improves = [(-(average_baseline[0] - average_bev[0])/(average_baseline[0]+0.00001)) * 100, 
            (-(average_baseline[1] - average_bev[1])/(average_baseline[1]+0.00001)) * 100, 
            (-(average_baseline[2] - average_bev[2])/(average_baseline[2]+0.00001)) * 100,
        ]
        ...
        texts = [
            ...
            ("CVPR 2024", f"minADE$_3$: {average_unc[0]:.2f} ({improves_unc[0]})", f"minFDE$_3$: {average_unc[1]:.2f} ({improves_unc[1]})", f"MR$_3$: {average_unc[2]:.2f} ({improves_unc[2]})"),
            ("ECCV 2024", f"minADE$_3$: {average_bev[0]:.2f} ({improves[0]})", f"minFDE$_3$: {average_bev[1]:.2f} ({improves[1]})", f"MR$_3$: {average_bev[2]:.2f} ({improves[2]})"),
            ("GT", ),
        ]
        ...
        for second_index, token in enumerate(tokens):
            if (first_index, second_index) not in track:
                ...
                key = track[(first_index, second_index)]
                value = data[key]
                file_path = bev_folder_path + 'scene-' + str(key) + '.pkl'
                ...
                # Stream
                    ...
                    bev_ = visualize_bev(data_content['predicted_map']['bev_features'], stream=True)

                data[int(key)]['predicted_map']['bev_features'] = bev_
                ...
                if divider.size != 0:
                    ...
                    plot_points_with_laplace_variances(n_divider[:,:,0], n_divider[:,:,1], divider_betas[:,:,0], divider_betas[:,:,1], 'orange', ax[0], False)
                    plot_points_with_laplace_variances(n_divider[:,:,0], n_divider[:,:,1], divider_betas[:,:,0], divider_betas[:,:,1], 'orange', ax[1], True)
                    plot_points_with_laplace_variances(n_divider[:,:,0], n_divider[:,:,1], divider_betas[:,:,0], divider_betas[:,:,1], 'orange', ax[2], False)
                if ped_crossing.size != 0:
                    ...
                    plot_points_with_laplace_variances(n_ped_crossing[:,:,0], n_ped_crossing[:,:,1], ped_crossing_betas[:,:,0], ped_crossing_betas[:,:,1], 'blue', ax[0], False)
                    plot_points_with_laplace_variances(n_ped_crossing[:,:,0], n_ped_crossing[:,:,1], ped_crossing_betas[:,:,0], ped_crossing_betas[:,:,1], 'blue', ax[1], True)
                    plot_points_with_laplace_variances(n_ped_crossing[:,:,0], n_ped_crossing[:,:,1], ped_crossing_betas[:,:,0], ped_crossing_betas[:,:,1], 'blue', ax[2], False)
                if boundary.size != 0:
                    n_boundary = np.array(normalize_lanes(x, y, boundary, heading))
                    n_boundary = n_boundary[:, :, [1, 0]]
                    n_boundary[:,:,0] = -n_boundary[:,:,0]
                    plot_points_with_laplace_variances(n_boundary[:,:,0], n_boundary[:,:,1], boundary_betas[:,:,0], boundary_betas[:,:,1], 'green', ax[0], False)
                    plot_points_with_laplace_variances(n_boundary[:,:,0], n_boundary[:,:,1], boundary_betas[:,:,0], boundary_betas[:,:,1], 'green', ax[1], True)
                    plot_points_with_laplace_variances(n_boundary[:,:,0], n_boundary[:,:,1], boundary_betas[:,:,0], boundary_betas[:,:,1], 'green', ax[2], False)
                ...
                for k , v in value['maptr_gt_map'].items():
                    ...
                    for j in range(len(x_pts)):
                        ax[3].plot(x_pts[j], y_pts[j], color=colors_plt[k], linewidth=1, alpha=0.8, zorder=-1)
                        ax[3].scatter(x_pts[j], y_pts[j], color=colors_plt[k], s=2, alpha=0.8, zorder=-1)
                ...
                del data[int(key)]['predicted_map']['bev_features']
...
def parse_args():
    ...
    parser.add_argument('--bev_results', type=str, required=True, help='trj prediction pkl of bev approach')
