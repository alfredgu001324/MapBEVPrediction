...
class new_reg_branch_laplace(nn.Module):
    def __init__(self, input_dim=256, hidden_dim=256):
        ...
        self.fc_means = nn.Linear(hidden_dim, 2)
        self.fc_stds = nn.Linear(hidden_dim, 2)
        self.fc_corr = nn.Linear(hidden_dim, 1)
...
class MapTRv2Head(DETRHead):
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False):
        ...
        outputs_pts_betas_one2one = []

        outputs_classes_one2many = []
        outputs_coords_one2many = []
        outputs_pts_coords_one2many = []
        outputs_pts_betas_one2many = []
        for lvl in range(hs.shape[0]):
            ...
            # tmp = self.reg_branches[lvl](hs[lvl])     # 4, 7400, 2
            # tmp = tmp[..., 0:2] if not self.z_cfg['gt_z_flag'] else tmp[..., 0:3]

            reg_out = self.reg_branches[lvl](hs[lvl])    # 4, 7400, 4
            if not self.z_cfg['gt_z_flag']:
                tmp = reg_out[:, :, 0:2]
                beta = reg_out[:, :, 2:4]
                outputs_pts_beta = beta.view(beta.shape[0], num_vec, self.num_pts_per_gt_vec, 2) # 4, 370, 20, 2
            else:
                tmp = reg_out[:, :, 0:3]
                beta = reg_out[:, :, 3:4]
                outputs_pts_beta = beta.view(beta.shape[0], num_vec, self.num_pts_per_gt_vec, 3) # 4, 370, 20, 3
            ...
            outputs_pts_betas_one2one.append(outputs_pts_beta[:, 0:self.num_vec_one2one])
            ...
            outputs_pts_betas_one2many.append(outputs_pts_beta[:, self.num_vec_one2one:])

        outputs_classes_one2one = torch.stack(outputs_classes_one2one)
        outputs_coords_one2one = torch.stack(outputs_coords_one2one)
        outputs_pts_coords_one2one = torch.stack(outputs_pts_coords_one2one)
        outputs_pts_betas_one2one = torch.stack(outputs_pts_betas_one2one)

        outputs_classes_one2many = torch.stack(outputs_classes_one2many)
        outputs_coords_one2many = torch.stack(outputs_coords_one2many)
        outputs_pts_coords_one2many = torch.stack(outputs_pts_coords_one2many)
        outputs_pts_betas_one2many = torch.stack(outputs_pts_betas_one2many)
        ...
        outs = {
            ...
            'all_pts_betas': outputs_pts_betas_one2one,
            'enc_cls_scores': None,
            'enc_bbox_preds': None,
            'enc_pts_preds': None,
            'depth': depth,
            ...
            "one2many_outs": dict(
                ...
                all_pts_betas=outputs_pts_betas_one2many,
                enc_cls_scores=None,
                enc_bbox_preds=None,
                enc_pts_preds=None,
                seg=None,
    ...
    def loss_single(self,
        ...
        pts_betas_list = [pts_betas[i] for i in range(num_imgs)]
        ...
        pts_betas = pts_betas.reshape(-1, pts_betas.size(-2), pts_betas.size(-1)) 
        out = torch.cat([pts_preds, pts_betas], dim=2)
    ...
    def loss(self,
        ...
        all_pts_betas = preds_dicts['all_pts_betas']
        ...
        losses_cls, losses_bbox, losses_iou, losses_pts, losses_dir = multi_apply(
            self.loss_single, all_cls_scores, all_bbox_preds,all_pts_preds, all_pts_betas,
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        bev_embed = preds_dicts['bev_embed']
        ...
        for i in range(num_samples):
            ...
            ret_list.append([bboxes, scores, labels, pts, betas, bev, raw_scores])
