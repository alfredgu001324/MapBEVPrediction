...
## not accurate
def denormalize_2d_betas(betas, pc_range):
    new_betas = betas.clone()
    new_betas[..., 0] = betas[..., 0] * (pc_range[3] - pc_range[0])
    new_betas[..., 1] = betas[..., 1] * (pc_range[4] - pc_range[1])
    return new_betas
...
class MapTRNMSFreeCoder(BaseBBoxCoder):
    ...
    def decode_single(self, cls_scores, bbox_preds, pts_preds, pts_betas):
        """Decode bboxes.
        Args:
            cls_scores (Tensor): Outputs from the classification head, \
                shape [num_query, cls_out_channels]. Note \
        ...
        # MapTRv2
        class_index = indexs // 3

        # MapTRv2 Centerline
        # class_index = indexs // 4
        # new_cls_scores = cls_scores[class_index]

        labels = indexs % self.num_classes
        bbox_index = indexs // self.num_classes
        bbox_preds = bbox_preds[bbox_index]
        pts_preds = pts_preds[bbox_index]
        pts_betas = pts_betas[bbox_index]
        ...
        final_pts_betas = denormalize_2d_betas(pts_betas, self.pc_range)
        ...
        if self.post_center_range is not None:
            ...
            predictions_dict = {
                ...
                'pts': pts,
                'betas': betas, 
                'raw_scores': new_cls_scores[mask],
            }
    ...
    def decode(self, preds_dicts):
        ...
        all_pts_betas = preds_dicts['all_pts_betas'][-1]
        batch_size = all_cls_scores.size()[0]
        predictions_list = []
        for i in range(batch_size):
            predictions_list.append(self.decode_single(all_cls_scores[i], all_bbox_preds[i], all_pts_preds[i], all_pts_betas[i]))
        return predictions_list
