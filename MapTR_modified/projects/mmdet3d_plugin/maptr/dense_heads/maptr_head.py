...
class MapTRHead(DETRHead):
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False):
        ...
        for lvl in range(hs.shape[0]):
            ...
            reg_out = self.reg_branches[lvl](hs[lvl])    # 4, 1000, 4
            # breakpoint()
            tmp = reg_out[:, :, 0:2]
            beta = reg_out[:, :, 2:4]

            outputs_pts_beta = beta.view(beta.shape[0], self.num_vec, self.num_pts_per_gt_vec, 2) # 4, 50, 20, 2
            # breakpoint()
            ...
            outputs_pts_betas.append(outputs_pts_beta)

        outputs_classes = torch.stack(outputs_classes)
        outputs_coords = torch.stack(outputs_coords)
        outputs_pts_coords = torch.stack(outputs_pts_coords)
        outputs_pts_betas = torch.stack(outputs_pts_betas)    # 6, 4, 50, 20, 3
        # breakpoint()
        outs = {
            'bev_embed': bev_embed,
            ...
            'all_pts_betas': outputs_pts_betas,
            'enc_cls_scores': None,
            'enc_bbox_preds': None,
            'enc_pts_preds': None
        }
    ...
    def loss_single(self,
        ...
        pts_preds = pts_preds.reshape(-1, pts_preds.size(-2), pts_preds.size(-1))    #200, 20, 2
        pts_betas = pts_betas.reshape(-1, pts_betas.size(-2), pts_betas.size(-1))    #200, 20, 2
