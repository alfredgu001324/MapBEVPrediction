...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if map_model == "MapTR":
            self.vit_encoder = ViT(image_size = (200, 100),
                                    patch_size = (20, 10),
                                    num_classes = 128,
                                    pool = 'mean',
                                    channels = 256,
            ...
            self.vit_encoder = ViT(image_size = (100, 50),
                                    patch_size = (10, 5),
                                    num_classes = 128,
                                    pool = 'mean',
                                    channels = 256,  
            ...
            logger.error("Invalid map_model specified: %s", map_model)
            sys.exit(1)
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        vit_embed = self.vit_encoder(data.clone())
        ...
        concatenated_features = torch.cat([out, vit_embed], dim=1)     
        final_output = self.processing_layer(concatenated_features)
...
class ALEncoder(MessagePassing):
    ...
    def __init__(self,
        ...
        self.betas_embed = nn.Parameter(torch.Tensor(2, embed_dim))
    ...
    def forward(self,
                ...
                size: Size = None) -> torch.Tensor:
        x_lane, x_actor = x
        x_actor = x_actor + self._mha_block_2(self.norm1(x_actor), x_lane, edge_index, edge_attr, rotate_mat, size)
    ...
    def message(self,
            ...
            if rotate_mat is None:
                ...
                x_j = self.lane_embed([torch.bmm(x_j[:,:2].unsqueeze(-2), rotate_mat).squeeze(-2),           # 19456 x 128
                                    torch.bmm(edge_attr[:,:2].unsqueeze(-2), rotate_mat).squeeze(-2)])
