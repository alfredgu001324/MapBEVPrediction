...
class new_reg_branch_laplace(nn.Module):
    def __init__(self, input_dim=512, hidden_dim=1024):
        ...
        self.fc_means = nn.Linear(hidden_dim, 40)
        self.fc_stds = nn.Linear(hidden_dim, 40)
...
class MapDetectorHead(nn.Module):
    ...
    def propagate(self, query_embedding, img_metas, return_loss=True):
        ...
        for i in range(bs):
            ...
            if is_first_frame:
                ...
                reg_output = self.reg_branches[-1](query_memory_updated)
                pred = reg_output[:, 0:40].sigmoid() # (num_prop, 2*num_pts)
                betas = reg_output[:, 40:80]
                ...
                if return_loss:
                    ...
                    # (num_prop, 2*num_pts)
                    # trans_loss += self.loss_reg(pred, normed_targets, weights, avg_factor=1.0)  # pred.shape= , normed_targets.shape=
                    # breakpoint()
                    # trans_loss += self.loss_reg(pred, normed_targets, weights, avg_factor=1.0)  
                    trans_loss += self.loss_reg(reg_output, normed_targets, weights, avg_factor=1.0)  
    ...
    def forward_train(self, bev_features, img_metas, gts):
        ...
        inter_queries, init_reference, final_inter_references = self.transformer(
        ...
        inter_references = []
        for i in range(len(final_inter_references)):
            inter_references.append(final_inter_references[i][:, :, :, 0:2])      
        betas = []
        for i in range(len(final_inter_references)):
            betas.append(final_inter_references[i][:, :, :, 2:4])
        ...
        for i, (queries) in enumerate(inter_queries):
            ...
            # breakpoint()
            reg_points = reg_points.contiguous().view(bs, -1, 2*self.num_points) # (bs, num_q, 2*num_points)
            beta = beta.contiguous().view(bs, -1, 2*self.num_points) # (bs, num_q, 2*num_points)
            # breakpoint()
        ...
        Outs:
            preds_dict (list[dict]):
                ...
                scores (Tensor):
                    [bs, num_query,]
        '''
        bev_features_old = bev_features.clone()
        bev_features = self._prepare_context(bev_features)
    ...
    def loss_single(self,
        ...
        pred_lines = torch.cat(preds['lines'], dim=0)  #400, 40
        betas = torch.cat(preds['betas'], dim=0)       # 400, 40
        ...
        loss_reg = self.loss_reg(torch.cat([pred_lines, betas], dim=-1), gt_lines, line_weights, avg_factor=num_total_pos)
    ...
    def post_process(self, preds_dict, tokens, thr=0.0):
        ...
        for i in range(bs):
            ...
            tmp_betas = tmp_betas.view(num_preds, num_points2//2, 2)
            # focal loss
            if self.loss_cls.use_sigmoid:
